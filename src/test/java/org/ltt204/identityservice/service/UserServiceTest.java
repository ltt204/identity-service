...
class UserServiceTest {
    ...
    @Nested
    @DisplayName("Test Service with User Role")
    @WithMockUser(username = "john", authorities = {"UPDATE_DATA"}, password = "123456789")
    class UserServiceTestWithUserRole {
        @Test
        ...
        void createUser_userExisted_fail() {
            ...
            var appException = new AppException(
                    ErrorCode.CONFLICT.withErrorCode("User already existed")
            );
            // WHEN
...
//            user.setUsername("john"); // Make sure usernames match
//
...
//            when(userService.findById(anyString())).then(
//                    invocation -> {
//                        throw new AppException(ErrorCode.NOT_FOUND.withErrorCode("User not found"));
//                    }
//            );
//
//            var exception = assertThrows(AppException.class, () -> userService.findById(anyString()));
//
//            assertThat(exception.getError().getCode()).isEqualTo(3001);
//        }
        ...
        void updateUser_userNotExisted_fail() {
            ...
            var exception = assertThrows(AppException.class, () -> {
                throw new AppException(
                        ErrorCode.NOT_FOUND.withErrorCode("User not found")
                );
            });
    ...
    @Nested
    @DisplayName("Test Service with Admin Role")
    @WithMockUser(username = "john", roles = {"ADMIN"})
    class UserServiceTestWithAdminRole {
        @Test
